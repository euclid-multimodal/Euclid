# Generating Multimodal Synthetic Geometric dataset using Symbolic Engine

This is the code space for the geometry image generation engine. [test.ipynb](alphageometry/test.ipynb) is the current version of it. To run the jupyter notebook, create a new environment and install the packages: 
```
...
Currently, the code can generate geometry shapes with arbitrary number of randomly sampled clauses and visualize them. 
Several problems are still to be solved:
1. Sometimes points get too close to the margin and the letters disappeared.
...
4. There are still some errors happening.

Most of the relevant codes are included in [test.ipynb](test.ipynb)
